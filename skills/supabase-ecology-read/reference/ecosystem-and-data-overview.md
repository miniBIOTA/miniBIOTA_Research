...
> Migration warning: this file preserves dated ecosystem and data context from `archive/superseded/ecosystem_and_data_overview.md`. Treat counts, statuses, "current" language, "thriving", "established", "stable", population outcomes, active loops/threads, species status, chronicle state, and biome condition as historical reference only. Verify current structured state in Supabase before reuse.
